...
var _getById = function(f, hid, cb){
	...
	log.debug(flowstr);
	db.footers.getById(f, hid, function(e,r){
		if(e){
...
var _getByTplId = function(f, tid, cb){
	...
	log.debug(flowstr);
	db.footers.getByTplId(f, tid, function(e,r){
		if(e){
			log.debug(flowstr + "error:" + e);
			cb(e)
		}else{
...
var _upload = function(f, tid, file, cb){
	...
	log.debug(flowstr);
	db.footers.store(f, {html: file, templates_idtemplates: tid}, function(e,r){
		if(e){
...
var _update = function(f, hid, file, cb){
	...
	log.debug(flowstr);
	db.footers.update(f, {html: file, hid: hid}, function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e)
		}else{
...
module.exports = {
	...
	create : function(req, res, next){
		...
		log.info(flowstr + "starting");
		if(req.body){
			var path = c.BE_PATH+"/uploads/tmp_f.html"
