...
var _getByTableId = function(f, tid, cb){
	var flowstr = f + "-" + handler + "-" + "_getByTableId:";
	log.debug(flowstr);
	db.fields.getByTableId(f,tid,function(e,r){
		if(e){
...
var _getByTplId = function(f, tid, cb){
	var flowstr = f + "-" + handler + "-" + "_getByTableId:";
	log.debug(flowstr);
	db.fields.getByTplId(f,tid,function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e);
		}else{
...
var _create = function(f, payload, cb){
	...
	log.debug(flowstr);
	db.fields.create(f, payload, function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e);
		}else{
...
var _delete = function(f, tableid, cb){
	var flowstr = f + "-" + handler + "-" + "_replace:";
	log.debug(flowstr);
	db.fields.delete(f, tableid, function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e);
		}else{
...
module.exports = {
	get : function(req, res, next){
		...
		if(req.params.tableid){
			_getByTableId(f, req.params.tableid, function(e,r){
				if(e){
	...
	delete : function (req, res, next){
		...
		if(req.params.tableid){
			_delete(f, req.params.tableid, function(e,r){
				if(e){
					log.error(flowstr + "500 - Internal Error, " + e);
					res.writeHead(500);
				}else{
	...
	getByTpl : function(req, res, next){
		var f = res.locals.flowid;
		var flowstr = f + "-" + handler + "-" + "get:";
