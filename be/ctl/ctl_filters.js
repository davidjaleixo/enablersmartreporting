...
var _create = function(f, payload, cb){
	...
	log.debug(flowstr);
	db.filters.create(f, payload, function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e);
		}else{
...
var _delete = function(f, filterid, cb){
	var flowstr = f + "-" + handler + "-" + "_delete:";
	log.debug(flowstr);
	db.filters.delete(f, filterid, function(e,r){
		if(e){
			log.error(flowstr + "error:" + e);
			cb(e);
		}else{
...
module.exports = {
	...
	delete : function (req, res, next){
		...
		if(req.params.fid){
			_delete(f, req.params.fid, function(e,r){
				if(e){
					log.error(flowstr + "500 - Internal Error, " + e);
					res.writeHead(500);
				}else{
